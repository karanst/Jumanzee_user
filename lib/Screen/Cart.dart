...
import 'package:Jumanzee/Provider/CartProvider.dart';
import 'package:Jumanzee/Provider/SettingProvider.dart';
import 'package:Jumanzee/Provider/UserProvider.dart';
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Widget listItem(int index, List<SectionModel> cartList) {
    ...
    for (int j = 0;
    j < cartList[index].productList![0].varients!.length;
    j++) {
      if (cartList[index].varientId ==
          cartList[index].productList![0].varients![j].id) selectedVarientPos = j;
    }
    ...
    // print("IMGAES++++++++++++++++++${cartList[index].productList![0].prVarientList![selectedPos].images![0]}");
    cartList[index].perItemPrice = price.toString();
    //print("qty************${cartList.contains("qty")}");
    // print("cartList**rererer****${price.toString()}");
          ...
          children: [
            Card(
              ...
              child: Row(
                children: <Widget>[
                  Hero(
                      tag: "$index${cartList[index].productList![0].varients![selectedVarientPos].id}",
                      child: Stack(
                        children: [
                          ClipRRect(
                              ...
                              child: Stack(children: [
                                Card(
                                  child: Container(
                                    ...
                                    height: 100,
                                    child: FadeInImage(
                                      image: cartList[index].productList![0].varients![selectedVarientPos].images!.isEmpty
                                          ? CachedNetworkImageProvider(
                                          "${cartList[index].productList![0].image}")
                                          :CachedNetworkImageProvider(
                                          "${cartList[index].productList![0].varients![selectedVarientPos].images![0]}"),
                                      height: 125.0,
                                      width: 110.0,
                                      fit: BoxFit.fill,
                                      imageErrorBuilder: (context, error, stackTrace) {
                                        print("ERORR MSG++++++++++++ $error");
                                        return erroWidget(125);
                                      },
  ...
  Widget cartItem(int index, List<SectionModel> cartList) {
    int selectedPos = 0, selectedVarientPos = 0;
    for (int i = 0;
        i < cartList[index].productList![0].prVarientList!.length;
        i++) {
      if (cartList[index].varientId ==
              ...
              children: <Widget>[
                Hero(
                    ...
                    ClipRRect(
                        borderRadius: BorderRadius.circular(7.0),
                        child: FadeInImage(
                          image: cartList[index].productList![0].varients![selectedVarientPos].images!.isNotEmpty
                              ? CachedNetworkImageProvider(
                              "${cartList[index].productList![0].varients![selectedVarientPos].images![0]}")
                              :CachedNetworkImageProvider(
                              "${cartList[index].productList![0].image!}"),
  ...
  Future<void> _getCart(String save) async {
    print("CUR_USERID:::"+ CUR_USERID.toString());
    print("CUR_USERID:save::"+ save);
    _isNetworkAvail = await isNetworkAvailable();
    if (_isNetworkAvail) {
      try {
        ...
        print("is On off===========> : $getdata");
        print("Seller Id-----------> $sellerId");
        print("Cart Data================> : $msg");

        print("Cart Data=====error===========> : $error");
        if (!error) {
          //print("Cart Data=====getdata===========> :"+ getdata);
          print("Cart Data=====data.tostring===========> :"+ getdata.toString());
  ...
  void _handlePaymentError(PaymentFailureResponse response) {
    print("FJHFJUJFv========== ${response.message.toString()}");
